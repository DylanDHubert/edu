...
export async function POST(request: NextRequest) {
  try {
    ...
    const { teamId, fileName, fileSize } = await request.json();

    // Validate required fields
    if (!teamId || !fileName || !fileSize) {
      return NextResponse.json(
        { error: 'Team ID, file name, and file size are required' },
        { status: 400 }
      );
    }
    ...
    const fileExtension = fileName.toLowerCase().match(/\.(xlsx|xls|csv)$/)?.[0] || '.xlsx';
    const uniqueFileName = `${timestamp}_${random}${fileExtension}`;
    const filePath = `teams/${teamId}/inventory/${uniqueFileName}`;
