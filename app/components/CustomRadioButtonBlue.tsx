...
interface CustomRadioButtonBlueProps {
  name: string;
  value: string;
  checked: boolean;
...
export default function CustomRadioButtonBlue({
  name,
  value,
  checked,
  onChange,
  ...
  return (
    <label className={`flex items-center gap-3 cursor-pointer group ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      <div className="relative flex-shrink-0">
        ...
        />
        {/* CUSTOM RADIO BUTTON VISUAL - BLUE VERSION */}
        <div className={`
          w-5 h-5 rounded-full border-2 transition-all duration-200 flex items-center justify-center relative
          ${checked 
            ? 'border-blue-500 bg-blue-500' 
            : 'border-slate-400 bg-slate-600 group-hover:border-blue-400 group-hover:bg-slate-500'
          }
          ${disabled ? 'border-slate-600 bg-slate-800' : ''}
        `} style={{
          backgroundColor: checked ? '#3b82f6' : '#475569',
          boxShadow: checked ? '0 0 0 3px rgba(59, 130, 246, 0.3), 0 4px 6px -1px rgba(0, 0, 0, 0.1)' : 'none'
        }}>
